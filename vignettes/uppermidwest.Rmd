...
pol_table %>% dplyr::select(-variable.element, -variable.context, -alias) %>% distinct %>% DT::datatable(options = list(pageLength=10))
...
veg_trans <- translate_taxa(veg_mean, 
                            veg_table,
                            id_cols = colnames(veg_mean)[1:4])
